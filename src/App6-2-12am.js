...
alert("App.fetchData");
		restService.getData({offset: offset, pageSize: offset + this.pageSize, sortField: this.state.sortField, sortOrder: order, searchValue: this.state.searchValue});
/*********************
...
console.log("page=" + page + " current=" + this.current + " lastPage=" + this.state.lastPage);
	...
	render() {
		...
		for (var i = 0; i < this.state.dataSize/this.pageSize; i++) {
			...
			elements.push(<li className="page-item" onClick={this.sendRequest.bind(this, i * this.pageSize)}><a className="page-link" key={i} href="#">{i + 1}</a></li>);
		}
		...
		return (
			...
			<nav className="navbar navbar-expand-sm bg-light navbar-light">
				...
				<div className="collapse navbar-collapse right" id="nav-content">  
						<input id="search" type="text" value={this.state.searchValue} className="form-control" onChange={this.updateInput}></input>
						<button className="btn" onClick={this.sendRequest.bind(this, 'search')}>Search Name</button>
				</div>
			</nav>
			<br/>
